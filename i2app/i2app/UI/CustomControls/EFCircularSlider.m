...
#define kStartingAngle 210
#define kBoundaryAngle 150
#define kConvertRatio 3
...
- (void)setValueInPercentage:(int)percentage {
    // AngleValue is angle from angleFromNorth(210) to current value
    // if percentage is 50 angle will be 150
    
    float angleValue = percentage * kConvertRatio;
    
    if (angleValue < kBoundaryAngle) {
        //original angle = 210 +angle
        self.angleFromNorth = kStartingAngle +angleValue;
    }
    
    else if (angleValue >= kBoundaryAngle) {
        // original angle = angle - 150
        self.angleFromNorth = angleValue - kBoundaryAngle;

    }
    [self setNeedsDisplay];
}

- (void)toggleStateOfControl {
    if ((previousAngle != (58.6f * 360)/(self.maximumValue - self.minimumValue)) && previousAngle != -1) { // !0%
        self.angleFromNorth = previousAngle;
        previousAngle = -1;
    }
    else {
        previousAngle = self.angleFromNorth;
        self.angleFromNorth = (58.6f * 360)/(self.maximumValue - self.minimumValue);
    }
    [self setNeedsDisplay];
}
 ...
 */
- (float)currentValue {
    int angleFromNorth = (self.angleFromNorth+150)%360;
    
    float percentValue = (angleFromNorth * (self.maximumValue - self.minimumValue))/360.0f;
    percentValue = ((int)percentValue%100) + (percentValue - (int)percentValue);
    if (directionToLock == 150) {
        percentValue = 300/3.6;
    }
    else if (directionToLock == 210) {
        percentValue = 0.0f;
    }
    
    percentValue = (percentValue * 100.0f) / (300/3.6);
...
- (CGFloat)radius {
    if (_radius == kFitFrameRadius) {
        ...
        CGFloat minimumDimension = MIN(self.bounds.size.height-self.heightPadding * 2, self.bounds.size.width - self.widthPadding * 2);
...
- (void)drawRect:(CGRect)rect {
    ...
    int angleFromNorth = (self.angleFromNorth+150)%360;
    if ((directionToLock == 210 && (angleFromNorth > 300 && angleFromNorth < 330)) || (directionToLock == 150 && angleFromNorth >= 330) || (directionToLock == 210 && angleFromNorth > 10) || (directionToLock == 150 && angleFromNorth < 290)) {
        self.angleFromNorth = directionToLock;
    }
    else {
        directionToLock = 0;
        if (angleFromNorth >= 330 && directionToLock%210 == 0) {
            directionToLock = 210;
            self.angleFromNorth = 210;
        }
        if ((angleFromNorth > 300 && angleFromNorth < 330) && directionToLock%150 == 0) {
            directionToLock = 150;
            self.angleFromNorth = 150;
        }
    }
...
- (void)drawLine:(CGContextRef)ctx {
    ...
    [EFCircularTrig drawUnfilledArcInContext:ctx
                                      ...
                                      radius:self.radius
                                   lineWidth:self.lineWidth
                          fromAngleFromNorth:210
                            toAngleFromNorth:self.angleFromNorth
                                  withShadow:YES
                         withBrightnessLevel:(self.shouldUpdateBrightness ? self.currentValue - 10 : 10)];
}
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if ([self.delegate respondsToSelector:@selector(isSliding)]) {
        [self.delegate isSliding];
    }
    return YES;
}
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if ([self.delegate respondsToSelector:@selector(endSliding)]) {
        [self.delegate endSliding];
    }
}
