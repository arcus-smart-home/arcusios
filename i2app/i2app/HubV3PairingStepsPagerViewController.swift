//
//  HubV3PairingStepsPagerViewController.swift
//  i2app
//
...
internal protocol HubV3PairingStepsPagerDelegate: class {
...
internal class HubV3PairingStepsPagerViewController: UIPageViewController,
UIPageViewControllerDelegate {

  fileprivate var stepViewControllers: [UIViewController] = []
  weak var stepsDelegate: HubV3PairingStepsPagerDelegate?
  ...
  func goNextPage() -> Bool {
    if getCurrentPageIndex() < stepViewControllers.count - 1 {
      ...
      stepsDelegate?.onPageChanged(isLastPage: isLastPage(),
                                   stepViewModel: (nextVc as? HubV3PairingInstructionViewController)?.step ?? nil)
      return true
    } else {
      return false
    }
  ...
  func goPreviousPage() -> Bool {
    if getCurrentPageIndex() > 0 {
      ...
      stepsDelegate?.onPageChanged(isLastPage: isLastPage(),
                                   stepViewModel: (prevVc as? HubV3PairingInstructionViewController)?.step ?? nil)
      return true
    } else {
      return false
    }
  ...
  func pageViewController(_ pageViewController: UIPageViewController,
    ...
    stepsDelegate?.onPageChanged(isLastPage: isLastPage(),
                                 stepViewModel: (vc as? HubV3PairingInstructionViewController)?.step ?? nil)
  }

}

extension HubV3PairingStepsPagerViewController: UIPageViewControllerDataSource {
...
extension HubV3PairingStepsPagerViewController: HubIDGettable {
