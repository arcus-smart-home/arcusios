...
@implementation ArcusHyperLabel
...
- (NSDictionary *)getLinkAttributeDefault {
    return @{NSForegroundColorAttributeName: self.hyperLabelLinkColorDefault,
             NSUnderlineStyleAttributeName: @(NSUnderlineStyleSingle)};
}

- (NSDictionary *)getLinkAttributeHighlighted {
    return @{NSForegroundColorAttributeName: self.hyperLabelLinkColorHighlighted,
             NSUnderlineStyleAttributeName: @(NSUnderlineStyleSingle)};
}
...
- (void)setLinkForRange:(NSRange)range withLinkHandler:(void(^)(ArcusHyperLabel *label, NSRange selectedRange))handler {
	[self setLinkForRange:range withAttributes:[self getLinkAttributeDefault] andLinkHandler:handler];
}
...
- (void)setLinkForSubstring:(NSString *)substring withAttribute:(NSDictionary *)attribute andLinkHandler:(void(^)(ArcusHyperLabel *label, NSString *substring))handler {
	...
	if (range.length) {
		[self setLinkForRange:range withAttributes:attribute andLinkHandler:^(ArcusHyperLabel *label, NSRange range){
...
- (void)setLinkForSubstring:(NSString *)substring withLinkHandler:(void(^)(ArcusHyperLabel *label, NSString *substring))handler {
	[self setLinkForSubstring:substring withAttribute:[self getLinkAttributeDefault] andLinkHandler:handler];
}

- (void)setLinksForSubstrings:(NSArray *)linkStrings withLinkHandler:(void(^)(ArcusHyperLabel *label, NSString *substring))handler {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	for (UITouch *touch in touches) {
		...
		if (rangeValue) {
			...
			[attributedString addAttributes:[self getLinkAttributeHighlighted] range:range];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	for (UITouch *touch in touches) {
		...
		if (rangeValue) {
			void(^handler)(ArcusHyperLabel *label, NSRange selectedRange) = self.handlerDictionary[rangeValue];
