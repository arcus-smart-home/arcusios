...
@implementation DoorLockScheduledEventModel
...
- (BOOL)getLockState {
    if (![self.attributes objectForKey:kAttrDoorLockLockstate] || ![self.attributes[kAttrDoorLockLockstate] isKindOfClass:[NSString class]] ) {
        return NO;
    }
    
    return [self.attributes[kAttrDoorLockLockstate] isEqualToString:kEnumDoorLockLockstateLOCKED];
}

- (void)setLockState:(BOOL)lock {
    [self.attributes setObject:lock ? kEnumDoorLockLockstateLOCKED : kEnumDoorLockLockstateUNLOCKED forKey:kAttrDoorLockLockstate];
}
