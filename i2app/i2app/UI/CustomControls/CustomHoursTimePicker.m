...
@interface CustomHoursTimePicker () <UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation CustomHoursTimePicker {
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.maxMinutes = 24 * 60; // Set default to 24 hours
    }
    return self;
}
...
- (void)initialize {
    ...
    UILabel *_separatorHrs = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width/3.0f, self.bounds.size.height/2.8f, 80, 60)];
    [_separatorHrs styleSet:@"hrs" andButtonType:FontDataType_Medium_12_Black upperCase:YES];
    [self addSubview:_separatorHrs];
    
    UILabel *_separatorMins = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width/1.5f, self.bounds.size.height/2.8f, 80, 60)];
    [_separatorMins styleSet:@"mins" andButtonType:FontDataType_Medium_12_Black upperCase:YES];
...
- (void)valueChanged {
    ...
    if (self.delegate != nil) {
        [self.delegate onTimePickerChanged:self.date];
    }
}
...
- (NSString *)getString {
    ...
    if (hour == 0 && minute == 0) return @"0 minute";
    
    return [NSString stringWithFormat:@"%@%@ ", (hour > 0? [NSString stringWithFormat:@"%ld hours",(long)hour]:@""), (minute > 0? [NSString stringWithFormat:@" %ld minutes",(long)minute]:@"") ];
}
