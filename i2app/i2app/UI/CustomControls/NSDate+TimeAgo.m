...
// TODO: TMomchev: this whole category may not be needed or should be cleaned up to only
// has logic that will be used in the app (TBD)
#ifndef NSDateTimeAgoLocalizedStrings
#define NSDateTimeAgoLocalizedStrings(key) \
[[NSBundle mainBundle] localizedStringForKey:(key) value:@"" table:@"NSDateTimeAgo"]
#endif
...
- (NSString *)timeAgoSimple {
    ...
    NSInteger deltaSeconds = lround(fabs([self timeIntervalSinceDate:now]));
    NSInteger deltaMinutes = lround(deltaSeconds / 60.0f);
...
- (NSString *)dateTimeUntilNow {
    ...
    if (components.hour >= 6) {
        // if more than 6 hours ago, change precision
