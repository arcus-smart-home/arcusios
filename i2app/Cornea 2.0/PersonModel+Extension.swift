...
extension PersonModel {
  ...
  var phoneEmailDescription: String? {
  ...
  static func createPersonModelContainingAddressForModelId(_ modelId: String) -> PersonModel? {
    let personWithoutAddress = PersonModel(attributes: [kAttrId: modelId as AnyObject])
    let address = personWithoutAddress.getAddressForNamespace(Constants.personNamespace)

    return PersonModel(attributes: [kAttrId: modelId as AnyObject,
                                    kAttrAddress: address as AnyObject])
  }

  static func filterOwner(_ personList: [PersonModel]) -> [PersonModel]? {
    if let currentAccount = RxCornea.shared.settings?.currentAccount,
  ...
  static func personRelationArray() -> [[String: String]] {
  ...
  static func subrelationArrayForRelationKey(_ relationKey: String) -> [[String: String]]? {
